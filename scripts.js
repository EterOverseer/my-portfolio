...
/* ══════════════════════════════════════════
   FREE SOURCE (DYNAMIC GITHUB)
══════════════════════════════════════════ */
async function renderFreeSource() {
  const grid = document.getElementById('fs-grid');
  grid.innerHTML = '<div class="fs-card" style="grid-column: 1/-1; padding: 2rem; text-align: center;">Loading repositories...</div>';

  const langIcons = {
    'JavaScript': '⚡', 'HTML': '🌐', 'CSS': '🎨', 'Python': '🐍',
    'C': '⚙️', 'Shell': '🐚', 'TypeScript': '📘', 'Dockerfile': '🐳',
    'Go': '🐹', 'Rust': '🦀', 'Arduino': '🔌'
  };

  try {
    const res = await fetch('https://api.github.com/users/EterOverseer/repos?sort=updated&per_page=100');
    const repos = await res.json();
    const filtered = repos.filter(r => !r.fork && r.name !== 'my-portfolio');

    grid.innerHTML = '';
    if (!filtered.length) {
      grid.innerHTML = '<div class="fs-card">No public repositories found.</div>';
      return;
    }

    filtered.forEach((repo, i) => {
      const branch = repo.default_branch || 'main';
      const icon = langIcons[repo.language] || '📁';
      grid.innerHTML += `
      <div class="fs-card fade-in" style="animation-delay:${i * 0.05}s">
        <div class="fs-icon">${icon}</div>
        <div class="fs-title">${repo.name}</div>
        <div class="fs-desc">${repo.description || 'No description available'}</div>
        <div class="fs-meta">${repo.language || 'Code'} • ⭐ ${repo.stargazers_count}</div>
        <div style="display: flex; gap: 0.5rem; margin-top: 1rem;">
          <a href="${repo.html_url}" target="_blank" class="fs-btn" style="flex: 1; text-align: center; text-decoration: none;">GitHub</a>
          <a href="https://github.com/EterOverseer/${repo.name}/archive/refs/heads/${branch}.zip" target="_blank" class="fs-btn" style="flex: 1; text-align: center; text-decoration: none;">ZIP</a>
        </div>
      </div>`;
    });
  } catch (err) {
    console.error(err);
    grid.innerHTML = '<div class="fs-card">Failed to load repositories.</div>';
  }
}
